...
//' @export
// [[Rcpp::export]]
List optimize_walk_forward_hmm_cpp(const arma::mat &X_all,
  ...
  for (int ti = 0; ti < ntasks; ++ti) {
    ...
    arma::vec ret_vals = X_allwin.col(0); // returns used for scoring

    // Determine combos applicable here (no further restriction)
    std::vector<Combo> combos;
    for (auto &c : global_combos) combos.push_back(c);
    ...
    // Helper lambda to evaluate one combo
    auto eval_combo = [&](size_t ci, double &out_score, HMMModel &out_model, arma::uvec &out_states_all, Combo &out_combo) {
      ...
      try {
        ...
        return; // skipped combo
      }

      // decode on full window (train + OOS)
      arma::uvec states_candidate;
      try {
        states_candidate = viterbi_decode(mdl, X_allwin); // may be 0..K-1 or 1..K
      } catch(...) {
        return;
      }
      if (states_candidate.n_elem == 0) return;

      // --- IMPORTANT FIX: normalize labels to 1..K even if viterbi returns 0..K-1
      int min_label = (int) states_candidate.min();
      int max_label = (int) states_candidate.max();
      if (verbose) {
        Rcpp::Rcout << "Task " << ti << " combo ci=" << ci << " decoded labels min=" << min_label << " max=" << max_label << "\n";
      }
      if (min_label == 0) {
        for (uword ii = 0; ii < states_candidate.n_elem; ++ii) states_candidate(ii) = states_candidate(ii) + 1;
        // update min/max after shift
        min_label = (int) states_candidate.min();
        max_label = (int) states_candidate.max();
      }
      // check that labels fit expected range
      if (min_label < 1 || max_label > c.nstates) {
        // labels unexpected: skip this combo
        if (verbose) Rcpp::Rcout << "Skipping combo due to unexpected state labels (min=" << min_label << ", max=" << max_label << ", expected 1.." << c.nstates << ")\n";
        return;
      }

      // compute state_means and sds (vectorized, use arma)
      arma::vec state_means(c.nstates); state_means.fill(arma::datum::nan);
      arma::vec state_sds(c.nstates); state_sds.fill(arma::datum::nan);
      for (int k=1;k<=c.nstates;k++) {
        arma::uvec idx = find(states_candidate == (arma::uword)k);
        if (idx.n_elem > 0) {
          arma::vec vals = ret_vals.elem(idx); // vectorized extraction
          state_means(k-1) = arma::mean(vals);
          state_sds(k-1) = (vals.n_elem > 1) ? arma::stddev(vals, /*norm_type=*/0) : 0.0;
        }
      }
      ...
      arma::vec oos_counts(c.nstates); oos_counts.zeros();
      if (to >= from) {
        for (int t = from; t <= to; ++t) {
          int st = (int) states_candidate(t); // now should be 1..K
          if (st >= 1 && st <= c.nstates) oos_counts(st-1) += 1;
        }
      }

      // choose bull_states and bear_states with preference for presence in OOS
      ...
      if (!std::isnan(mean_bull) && !std::isnan(mean_bear)) {
        ...
        // penalize combos that do not produce both bull and bear in OOS (less extreme than before)
        score = -1e6 - ((nbull_obs + nbear_obs));
      }
    ...
    // --- Evaluate combos sequentially (safe) ---
    std::vector<double> scores(ncomb, -std::numeric_limits<double>::infinity());
    std::vector<arma::uvec> states_all_vec(ncomb); // keep for fallback inspection if needed
    ...
    for (size_t ci = 0; ci < ncomb; ++ci) {
      try {
        ...
        // Update global best immediately if valid
        if (std::isfinite(sc)) {
          if (sc > best_score) {
            best_score = sc;
            best_combo = outc;
            // assign best_model with move attempt (if supported)
            try {
              best_model = std::move(mdl);
            } catch(...) {
              try { best_model = mdl; } catch(...) { /* ignore copy failure */ }
            }
            best_states_all = stv;
          }
        }
      } catch(...) {
        // ignore combo failure to remain robust
      }
    }

    // fallback: if no valid best (all -inf), choose first successful combo/training
    if (!std::isfinite(best_score)) {
      ...
      for (size_t ci=0; ci<ncomb; ++ci) {
        if (states_all_vec[ci].n_elem > 0) {
          ...
          // try retrain model for this combo to populate best_model
          try {
            best_model = train_hmm_em(X_train, best_combo.nstates, maxit, tol, cov_reg, false);
          } catch(...) {
            // ignore if retrain fails, but we have states at least
          }
          found = true;
          break;
        }
      ...
      if (!found) {
        // last resort: train with minimal params (use first nstates_cand)
        ...
        try {
          ...
          if (best_states_all.n_elem > 0 && (int)best_states_all.min() == 0) {
            for (uword ii = 0; ii < best_states_all.n_elem; ++ii) best_states_all(ii) = best_states_all(ii) + 1;
          }
          best_combo = {ns, 1, 1};
    ...
    arma::vec state_means_best(Kbest); state_means_best.fill(arma::datum::nan);
    arma::vec state_sds_best(Kbest); state_sds_best.fill(arma::datum::nan);

    // Normalize best_states_all labels to 1..Kbest if necessary
    if (best_states_all.n_elem > 0) {
      int min_b = (int) best_states_all.min();
      if (min_b == 0) {
        for (uword ii = 0; ii < best_states_all.n_elem; ++ii) best_states_all(ii) = best_states_all(ii) + 1;
      }
    }
    ...
    for (int k=1;k<=Kbest;k++) {
      arma::uvec idx = find(best_states_all == (arma::uword)k);
      if (idx.n_elem > 0) {
        arma::vec vals = ret_vals.elem(idx);
        state_means_best(k-1) = arma::mean(vals);
        state_sds_best(k-1) = (vals.n_elem > 1) ? arma::stddev(vals, 0) : 0.0;
      }
    }
    ...
    arma::vec oos_counts_best(Kbest); oos_counts_best.zeros();
    int from = train_end;
    int to = predict_end - 1;
    if (to >= from) {
      for (int t = from; t <= to; ++t) {
    ...
    if (bull_states_best.empty() && bear_states_best.empty() && verbose) {
      Rcpp::Rcout << "Warning: no bull or bear states selected for task " << ti << " (K=" << Kbest << ").\n";
    }
    if (bull_states_best.empty() && verbose) {
      Rcpp::Rcout << "Note: no bull states selected for task " << ti << "\n";
    }
    if (bear_states_best.empty() && verbose) {
      Rcpp::Rcout << "Note: no bear states selected for task " << ti << "\n";
    }
